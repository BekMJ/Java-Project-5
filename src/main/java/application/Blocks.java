...
public class Blocks implements Comparable<Blocks>
{
	...
	public static LinkedHashMap<String, Integer> calUniqMiners(int block1, int block2) throws FileNotFoundException, IOException {

		LinkedHashMap<String, Integer> Unique = new LinkedHashMap<>();
		
		for (int i = blocks.indexOf(Blocks.getBlockByNumber(block1)); i < blocks.indexOf(Blocks.getBlockByNumber(block2)); i++) {
		...
		System.out.println("Number of unique Miners: " + Unique.size());
		System.out.println();
		System.out.println("Each unique Miner and its frequency:");
		for (int i = 0; i < Unique.size(); i++) {
			System.out.println("Miner Address: " + Unique.keySet().toArray()[i]);
			System.out.println("Miner Frequency: " + Unique.get(Unique.keySet().toArray()[i]));
			System.out.println();
		}
		return Unique;
	}
	...
	public static int [] getTimeDiff(Blocks first, Blocks second) {
		...
		int[] result = {diffInHours, diffInMinutes, diffInSeconds};
		return result;
	}
